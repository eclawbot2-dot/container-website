...
// Unverified until the *_VERIFIED flags in lib/config.ts flip — must NOT link out.
const FORBIDDEN = [
  /instagram\.com/i,
  /facebook\.com/i,
  /tiktok\.com/i,
...
for (const f of ['index.html', '404.html', 'sitemap.xml', 'robots.txt', 'favicon.svg', 'apple-touch-icon.png']) {
...
// 3. HTML pages: forbidden destinations, unknown external hosts, broken local images.
let pages = 0;
for (const file of htmlFiles(OUT)) {
  ...
  // (e.g. the footer's url(/images/warehouse.jpg)) must exist in the export.
  for (const m of html.matchAll(
    /(?:(?:href|src)="|url\()(\/images\/[^")]+|\/favicon[^")]*|\/apple-touch-icon[^")]*)["\)]/g,
  )) {
    const asset = decodeURIComponent(m[1].split('?')[0]);
    if (!existsSync(join(OUT, asset))) fail(`${rel}: references missing asset ${asset}`);
  }
}
if (pages < 4) fail(`only ${pages} HTML pages exported — expected home + events + 404`);
